...
# Usage: python3 /home/xinchang/pyscript/get_bipartite_from_tree.py --input_dir="/scratch/xinchang/hcyen/out/concatboot" --list_file="/scratch/xinchang/hcyen/list/testlist.list" --output_dir="/scratch/xinchang/hcyen/output"
...
def main():
    ...
    if not os.path.exists("%s" % args.output_dir) :
        output = os.makedirs("%s" % args.output_dir) 
    else:
    ...
    for in_file in inputs:
        ...
        if set(final_runs) == set(runs_list):
            try:
                shutil.copyfile(in_file, os.path.join("%s, %s" % (output, file_name)))
